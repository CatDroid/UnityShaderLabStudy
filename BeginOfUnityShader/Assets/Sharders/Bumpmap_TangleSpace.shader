Shader "Custom/Bumpmap_TangleSpace"
{
	...
	#pragma fragment frag
			...
			v2f vert(a2v input)
			{
				...
				// Unity在导入法线贴图的时候会自动将法线贴图压缩成DXT5nm格式
				// 这个格式的好处是它只使用ag两个通道来存放两个轴向的坐标值
				// 而另外一个轴向的坐标值由于是单位坐标可以通过1减去另外前两个轴向坐标的平方和来得到
				// 从而可以以同样的容量存放"更大尺寸的法线贴图"
				// 如果你是针对移动平台来开发游戏那么Unity不会为你压缩法线贴图，这意味着你还要使用RGB通道来解析法线贴图
				// https://www.cnblogs.com/Esfog/p/3608833.html
